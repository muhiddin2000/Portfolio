...
<header>
    ...
    <div class="user">
        <img :src="this.me?.image" alt="" />
        <h3 class="name">Muhiddin Malikov</h3>
...
<section class="home" id="home">
    ...
    <h3>Salom!</h3>
    <h1>Men <span>Muhiddin Malikov</span></h1>
    <p>
...
<section class="about" id="about">
    ...
    <div class="row">
        ...
        <div class="info">
            ...
            <h3> <span> Kasbim : </span> {{me.job}} </h3>
            <h3> <span> Til : </span> {{me.languages}} </h3>
            <a :href="me.resume" download target="_blank"><button class="btn"> CVni ko'rish <i class="fas fa-download"></i> </button></a>
        </div>

        <div class="counter">

            <div class="box">
                <span>{{me.profession}}</span>
...
<section class="portfolio" id="portfolio">
    ...
    <div class="box-container">
      ...
      <div class="box" v-for="portfolio in portfolios" :key="portfolio.id">
        ...
        <div class="content">
            <h3 class="ptitle">{{portfolio.name}}</h3>
...
<section class="contact" id="contact">
    ...
    <div class="row">
        ...
        <div class="content">
            ...
            <div class="info">
                <h3> <i class="fas fa-envelope"></i> {{info.gmail}} </h3>
                <h3> <i class="fas fa-phone"></i> {{info.phone_number}}</h3>
            
                <h3> <i class="fas fa-map-marker-alt"></i>{{info.adress}}</h3>
            </div>
...
export default {
    ...
    methods:{
       async SendMe(){
              ...
              await axios.post(`https://api.telegram.org/bot5854827077:AAF3uBG4deoitar-tXxY3M9JAA0TSNGbDRs/sendMessage?chat_id=5222731866&text=${this.name+'\nn'+this.message}`).then(res =>{
                toast({
                    pauseOnHover:true,
       ...
       async Oldinga(){
        ...
        async getme(){
            ...
            await axios.get(`${this.url}/info/`).then(res => {
                this.info = res.data.results[0]
                
               
            })
            await axios.get(`${this.url}/social/`).then(res => {
                this.link = res.data.results[0]
               
            })
            await axios.get(`${this.url}/portfolio/`).then(res => {
                this.portfolios = res.data.results[0],
                this.next = res.data.next,
                this.previous = res.data.previous,
                this.allpages = res.data.count,
                this.currentpage_num = res.data.current_page_num
